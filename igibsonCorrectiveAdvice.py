...
def concatenate_obs_actions(sampled_obs_actions, device):
    ...
    for _, action in sampled_obs_actions[1:]:
        final_actions = np.concatenate((final_actions, np.expand_dims(action, axis=0)), axis=0)
...
def train_ol_model(ol_agent, env, device, learning_rate, robot, iterations, train_every=1, max_replay_buffer=3072, batch_size=16):
    optimizer = None
    ...
    for iteration in range(iterations):
        ...
        while not done:
            ...
            if not ol_agent:
                ol_agent = CustomCombinedExtractor(env.observation_space, device, 26).to(device)
                ol_agent.train()
            ...
            if iteration % train_every == 0 and len(replay_buffer) > 0:
                training_samples = random.sample(replay_buffer, min(len(replay_buffer), batch_size))
                obs_batch, label_actions = concatenate_obs_actions(training_samples, device)
