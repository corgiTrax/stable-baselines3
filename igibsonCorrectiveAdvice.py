...
def train_ol_model(ol_agent, env, device, learning_rate, robot):
    optimizer = None
        ...
        while not done:
            ...
            if not ol_agent:
                ol_agent = OLNet_taskObs(
                    task_obs_dim=task_obs.size()[-1],
                    proprioception_dim=proprioception.size()[-1],
                ).to(device)
                ol_agent.train()
