...
class LunarLanderSceneGraphV2:
    ...
    def calculate_ucb(self, graph):
        ...
        # file1.write(f'{self.human_critic_average[tuple(graph)]}   {0.2 * math.sqrt(2 * self.given_feedback / self.state_counts[tuple(graph)])}\n')
        # file1.close() 
        return self.aRPE_average[tuple(graph)] + 0.02 * math.sqrt(2 * self.given_feedback / self.num_feedback_given[tuple(graph)])       
    ...
    def updateGraph(self, newState, action):
        ...
        return self.getCurrGraph != prev_graph, self.getUCBRank() <= 30
