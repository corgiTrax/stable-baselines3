...
class LunarLanderSceneGraph:
    ...
    def calculate_ucb(self, graph):
        ...
        # file1.close() 
        return abs(self.human_critic_average[tuple(graph)]) + 0.01 * math.sqrt(2 * self.given_feedback / self.state_counts[tuple(graph)])       
    ...
    def updateGraph(self, newState, action, human_critic_estimate, total_timesteps):
        ...
        return self.getCurrGraph(human_critic_estimate) != prev_graph, self.curr_prob, self.getStateRank() <= (int(self.total_feedback / self.given_feedback)), self.getUCBRank() <= 40
